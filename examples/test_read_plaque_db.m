...
datapath = '/home/wallenberg/Data/Amyloids/mouse_brain';

% make a list of the files
files = make_file_list(datapath, '*.raw');

% from the file names, create a list of ages
ages = zeros(size(files));
for k=1:numel(files)
    % This assumes 'age' does not appear in the path
    dummy = strsplit(files{k}, {'age'});
    dummy = strsplit(dummy{end}, '_');
    ages(k) = str2double(dummy{1});
end

% sort file list by age
[grades, sidx] = sort(ages, 'ascend');
files = files(sidx);
...
for k=1:numel(files)
    ...
    xlabel('X');
    ylabel('Y');
    title(['False-colour RGB image, age ' num2str(grades(k)) ' days'], 'FontSize', 15);
    subplot(2, 2, 3);
